        ...
        Box::new(move |ctx: &egui::Context, _frame: &mut eframe::Frame| {
            ...
            egui::CentralPanel::default().show(ctx, |ui| {
                ...
                let (response, painter) =
                    ui.allocate_painter(painter_size, egui::Sense::drag());
                ...
                if !current_path.is_empty() {
                    painter.add(Shape::Path(PathShape {
                        points: current_path.iter().map(|p| *p + rect.min.to_vec2()).collect(),
                        closed: false,
                        fill: Color32::TRANSPARENT,
...
fn export_to_image(paths: &Vec<Vec<Pos2>>, size: Vec2) {
    let mut img: GrayImage = ImageBuffer::from_pixel(size.x as u32, size.y as u32, image::Luma([0]));
    for path in paths {
...
fn draw_thick_line(img: &mut GrayImage, start: Pos2, end: Pos2, thickness: i32) {
    for i in -thickness..=thickness {
        for j in -thickness..=thickness {
            draw_line(img, Pos2 { x: start.x + i as f32, y: start.y + j as f32 }, Pos2 { x: end.x + i as f32, y: end.y + j as f32 });
        }
    }
}
